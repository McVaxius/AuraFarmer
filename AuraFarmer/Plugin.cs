...
using AuraFarmer.Windows;
using AuraFarmer.Services;
...
public sealed class Plugin : IDalamudPlugin
{
    ...
    public Plugin()
    {
        ...
        SetupDtrBar();
        UpdateDtrBar();
        Log.Information("[Aura Farmer] Plugin loaded.");
    }
    ...
    {
        if (dtrEntry == null) return; dtrEntry.Shown = Configuration.DtrBarEnabled; if (!Configuration.DtrBarEnabled) return; var g = Configuration.PluginEnabled ? Configuration.DtrIconEnabled : Configuration.DtrIconDisabled; var s = Configuration.PluginEnabled ? "On" : "Off"; dtrEntry.Text = Configuration.DtrBarMode switch { 1 => new SeString(new TextPayload($"{g} AF")), 2 => new SeString(new TextPayload(g)), _ => new SeString(new TextPayload("AF: " + s)), }; dtrEntry.Tooltip = new SeString(new TextPayload($"{PluginInfo.DisplayName} {s}. Click to toggle."));
    }
