...
using AuraFarmer.Models;

namespace AuraFarmer.Services;
...
public sealed class ConfigManager
{
    ...
    private void LoadAllAccounts() { try { foreach (var p in Directory.GetFiles(configDirectory, "*_AuraFarmer.json")) { var a = JsonSerializer.Deserialize<AccountConfig>(File.ReadAllText(p), JsonOptions); if (a != null && !string.IsNullOrWhiteSpace(a.AccountId)) accounts[a.AccountId] = a; } } catch (Exception ex) { log.Error(ex, "[Aura Farmer] Failed to load account configs."); } }
    private void SaveAccount(string id) { if (!accounts.TryGetValue(id, out var a)) return; try { File.WriteAllText(Path.Combine(configDirectory, $"{id}_AuraFarmer.json"), JsonSerializer.Serialize(a, JsonOptions)); } catch (Exception ex) { log.Error(ex, "[Aura Farmer] Failed to save account config."); } }
}
